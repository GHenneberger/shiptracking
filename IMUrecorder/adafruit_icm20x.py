...
class ICM20X:  # pylint:disable=too-many-instance-attributes
    ...
    _raw_accelgyro_data = Struct(_ICM20X_ACCEL_XOUT_H, ">hhhhhh")  # ds says LE :|
    ...
    def accelgyro(self):
        """The x, y, z acceleration values returned in a 3-tuple and are in m / s ^ 2."""
        self._bank = 0
        raw_accelgyro_data = self._raw_accelgyro_data
        #sleep(0.001)

        ax = self._scale_xl_data(raw_accelgyro_data[0])
        ay = self._scale_xl_data(raw_accelgyro_data[1])
        az = self._scale_xl_data(raw_accelgyro_data[2])
        wx = self._scale_gyro_data(raw_accelgyro_data[3])
        wy = self._scale_gyro_data(raw_accelgyro_data[4])
        wz = self._scale_gyro_data(raw_accelgyro_data[5])
